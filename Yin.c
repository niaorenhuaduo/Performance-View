...
void Yin_cumulativeMeanNormalizedDifference(Yin *yin){
     ...
     * the value in the autocorellation buffer with a cumulative mean of the normalized difference */
 ...
 */
int16_t Yin_absoluteThreshold(Yin *yin, int hz_hat){
    int16_t tau;
 ...
 */
float Yin_getPitch(Yin *yin, int16_t* buffer, int hz_hat){
    ...
    /* Step 3: Search through the normalised cumulative mean array and find values that are over the threshold */
    tauEstimate = Yin_absoluteThreshold(yin, hz_hat);
