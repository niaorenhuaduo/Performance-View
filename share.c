...
// #define SKIP_APP_STORE // no purchases through the app store --- just download without any payment and don't check for keys of purchased pieces
...
#define FRAMELEN 512     /* size of data chunk for evaluation: this is fixed, used for spectrogram generation */
...
typedef struct  {
 ...
 // LOC loc;  // location on staff
 // int staff; // which staff line is the note on
} SOLO_NOTE;
...
typedef struct {
  ...
  int running;
  int ret_val;
    int blank; // seems to be bug in compiler about reference the "name" field below.  
  char name[500];
  char *label;
  int   (*function)(void);
} BACKGROUND_INFO;
